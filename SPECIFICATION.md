...
The REQUIRED construction is full HKDF (RFC 5869, SHA-256): Extract then Expand, with the composite as input keying material, an empty (zero-filled) salt, and a domain-separation label as `info`, producing a 32-byte per-composite salt for each output:

```
salt_locator = HKDF-SHA256(IKM=composite, salt=∅, info="ghostbox/v1/locator", L=32)
salt_access  = HKDF-SHA256(IKM=composite, salt=∅, info="ghostbox/v1/access",  L=32)
```

This is full HKDF, NOT a bare `HMAC(label, composite)`. The bare-HMAC formulation is a distinct (and incorrect) construction: it does not match RFC 5869 and produces different output, so an implementation using it will not interoperate. The canonical TypeScript (`@noble/hashes` `hkdf`) and the Python reference (RFC 5869 Extract+Expand) produce byte-identical salts under the construction above, verified by frozen cross-language test vectors.
...
> **Reference implementation:** `src/transport.ts` (canonical). End-to-end with the Spirit Layer in `test-vectors/integration.mjs`; the unlinkability property is demonstrated by inspection in `test-vectors/verify_unlinkability.mjs`. The reference provides two encryption paths: the sealed box (ephemeral-static X25519 + XChaCha20-Poly1305) for the pre-session Lobby flow, giving sender anonymity but not forward secrecy; and, as of v0.4.0, the in-channel `RatchetSession` path, which routes per-message keys from the §6.2 symmetric ratchet through the committing envelope and is forward secret. The unlinkability demonstration is application-layer only — see §8.4 for the network-layer threat it does not address.
...
#### 6.1.1 Key commitment (IMPLEMENTED — `src/envelope.ts`)

XChaCha20-Poly1305 and AES-GCM provide confidentiality and integrity but **not key commitment**: an attacker who controls key material can craft a single ciphertext that decrypts validly under two different keys to two different plaintexts (the Invisible Salamanders / partitioning-oracle attack). This bites the Mediated Introduction discovery mode (§5), where one payload is handed to two recipients. Every AEAD operation MUST therefore be wrapped in a committing transform.
...
The AEAD is keyed by `subkey`, never by `K` directly. On receipt the client MUST recompute `commit` from its own key, compare in **constant time**, and **REJECT before AEAD decryption** on mismatch. This committing envelope is the canonical message envelope for every Specter-layer PUT. Reference: `src/envelope.ts`, exercised by `test-vectors/key-commitment.mts`.
...
> **Reference status (v0.3.1):** the symmetric-key ratchet **core is implemented** (`src/ratchet.ts`, exercised by `test-vectors/ratchet.mts`): a triple-DH handshake derives a shared session root, per-direction KDF chains give forward secrecy, and out-of-order / batched delivery is handled via skipped message keys capped at 64 (exceeding the cap forces re-handshake). It is a **symmetric** ratchet, not a DH ratchet: there is no per-message DH step and therefore no session-level post-compromise security — deliberate, since identity/Quad-Key compromise is permanent by design (§3, §6.4), so session-level self-healing would not close that wound. As of **v0.4.0** the ratchet core **is wired in**: `RatchetSession` (`src/transport.ts`, exercised end-to-end by `test-vectors/fs-transport.mts`) routes ratchet message keys through the committing envelope with the message-number header as authenticated associated data, and `src/statesync.ts` serializes the live `RatchetState` in a canonical byte-stable format for the §6.4 sync channel. Because deposits are unauthenticated (§4.2), the receive path is transactional: ratchet state commits only when a blob's commitment and AEAD authenticate, so forged headers cannot burn message numbers or poison the skipped-key cache. The sealed-box path remains for the pre-session Lobby flow (§5.7 step 2), where sender anonymity — not forward secrecy — is the required property. The §6.4 derivations:
>
> ```
> root         = HKDF(DH(id,ephₚ) ‖ DH(eph,idₚ) ‖ DH(eph,ephₚ), "ghostbox/v1/root", 32)
> sync_locator = HKDF(access_seed, "ghostbox/v1/sync-locator", L=16)
> sync_key_0   = HKDF(access_seed, "ghostbox/v1/state-sync",   L=32)
> sync_key_n   = HKDF(sync_key_{n-1}, "ghostbox/v1/state-sync", L=32)
> ```
>
> Remaining for a future revision: the Corporeal-Layer Lobby handshake that carries the ephemeral public keys (§5.7) — `establishSession` in `src/transport.ts` is the seam it plugs into — and independent cryptographic review of the ratchet, triple-DH, and the v0.4.0 wiring.
