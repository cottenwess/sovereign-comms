...
export async function deriveIdentity(
  ...
  try {
    ...
    const signSeed = hkdf(sha256, accessSeed, undefined, new TextEncoder().encode("ghostbox/v1/sign"), 32);
    const encSeed = hkdf(sha256, accessSeed, undefined, new TextEncoder().encode("ghostbox/v1/enc"), 32);
